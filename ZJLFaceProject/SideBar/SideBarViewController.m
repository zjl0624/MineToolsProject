...
- (void)pan:(UIPanGestureRecognizer *)panGes {
    ...
    NSLog(@"%f",p.x);
    if (panGes.state == 2) {
        if (CGRectGetMinX(self.navigationController.view.frame) + p.x >= 0 && CGRectGetMinX(self.navigationController.view.frame) + p.x <= CGRectGetWidth(_sideBar.frame)) {
            self.navigationController.view.frame = CGRectMake(CGRectGetMinX(self.navigationController.view.frame) + p.x, 0, CGRectGetWidth(self.view.frame), CGRectGetHeight(self.view.frame));
        ...
        if (CGRectGetMinX(self.navigationController.view.frame) + p.x > CGRectGetWidth(_sideBar.frame)/2) {
            [UIView animateWithDuration:0.25 animations:^{
                self.navigationController.view.frame = CGRectMake(CGRectGetWidth(_sideBar.frame), 0, CGRectGetWidth(self.view.frame), CGRectGetHeight(self.view.frame));
